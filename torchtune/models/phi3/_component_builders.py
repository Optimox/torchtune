...
def phi3_medium(
    ...
    self_attn = CausalSelfAttention(
        ...
        num_kv_heads=num_kv_heads,
        head_dim=head_dim,
        q_proj=nn.Linear(embed_dim, q_size, bias=False),
        k_proj=nn.Linear(embed_dim, kv_size, bias=False),
        v_proj=nn.Linear(embed_dim, kv_size, bias=False),
        output_proj=nn.Linear(embed_dim, embed_dim, bias=False),
        pos_embeddings=rope,
        kv_cache=None,
        max_seq_len=max_seq_len,
...
def lora_phi3_medium(
    ...
    *,
    ...
    self_attn = lora_phi3_medium_self_attention(
        lora_modules=lora_attn_modules,
        embed_dim=embed_dim,
        num_heads=num_heads,
        num_kv_heads=num_kv_heads,
...
def lora_phi3_medium_self_attention(
    lora_modules: List[LORA_ATTN_MODULES],
    *,
    # CausalSelfAttention args
    embed_dim: int,
    ...
    q_proj = (
        ...
        if "q_proj" in lora_modules
        else nn.Linear(embed_dim, q_size, bias=False)
    )
    k_proj = (
        LoRALinear(
            embed_dim,
        ...
        if "k_proj" in lora_modules
        else nn.Linear(embed_dim, kv_size, bias=False)
    )
    v_proj = (
        LoRALinear(
            embed_dim,
